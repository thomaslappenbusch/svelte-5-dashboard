...
<div class="w-full flex relative">
	<div
		class={`h-full fixed overflow-hidden flex flex-col bg-ui-bg justify-start items-start px-4 py-5 flex-shrink-0 border-r border-ui-br
	...
	>
		...
		<div class="flex flex-col gap-4 w-full mb-3 justify-end">
			{#each $navStore.alerts as item (item.id)}
				...
				>
					...
					>
						<svg
							class="text-ui-tx-3/20 group-hover:text-ui-tx-3 transition duration-200 {$isNavOpen
								? 'opacity-100'
					...
					<div
						class="absolute left-0 bottom-0 h-full w-[2px] {item.leftIcon === 'warning'
							? 'bg-red-600'
							: item.leftIcon === 'hazard'
								? 'bg-yellow-500'
								: item.leftIcon === 'avatar'
									? 'bg-green-500'
									: item.leftIcon === 'download'
										? 'bg-blue-500'
										: item.leftIcon === 'sync'
											? 'bg-purple-500'
											: 'bg-zinc-500'}"
					...
					<div class="h-full aspect-square flex items-center justify-center flex-shrink-0">
						{#if item.leftIcon === 'warning'}
							<svg
								class="text-red-600 p-[10px] ml-[2px]"
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 24 24"
							>
								<path
						...
						{:else if item.leftIcon === 'hazard'}
							<svg
								class="text-yellow-500 p-[10px] ml-[2px]"
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 24 24"
							>
								<path
						...
						{:else if item.leftIcon === 'success'}
							<svg
								class="text-green-500 p-[10px] ml-[2px]"
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 24 24"
								><path
									fill="currentColor"
						...
						{:else if item.leftIcon === 'download'}
							<svg
								class="text-blue-500 p-[10px] ml-[2px]"
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 24 24"
							>
								<path
									fill="currentColor"
									d="M12 15.575q-.2 0-.375-.062T11.3 15.3l-3.6-3.6q-.275-.275-.275-.7t.275-.7q.275-.275.7-.275t.7.275l1.9 1.9V4q0-.425.288-.713T12 3q.425 0 .713.288T13 4v8.2l1.9-1.9q.275-.275.7-.275t.7.275q.275.275.275.7t-.275.7l-3.6 3.6q-.15.15-.325.213t-.375.062M6 21q-.825 0-1.413-.588T4 19v-2q0-.425.288-.713T5 16q.425 0 .713.288T6 17v2h12v-2q0-.425.288-.713T19 16q.425 0 .713.288T20 17v2q0 .825-.588 1.413T18 21z"
								/>
							</svg>
						{:else if item.leftIcon === 'sync'}
							<svg
								class="text-purple-500 p-[10px] ml-[2px]"
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 24 24"
							>
								<path
		...
		<Avatar data={$navStore.profile} {handleLogout} />
	</div>
	<div
		class={`
