...
<div class="w-screen h-screen bg-ui-bg flex items-center justify-center relative">
    ...
    <div class="px-10 flex flex-col items-start justify-start w-[450px]">
        ...
        <div class="text-ui-tx text-sm mb-6">New to SvelteDashboard? <button onclick={() => goto("/auth/create-account")} class="text-ui-tx-3 underline opacity-80 hover:opacity-100 hover:text-ui-tx-2 transition ml-[1px]">Create account</button></div>
        ...
        <div class="flex items-center justify-between w-full mt-2">
            ...
            <button onclick={() => goto("/auth/forgot-password")} class="text-ui-tx-3 text-sm underline opacity-80 hover:opacity-100 hover:text-ui-tx-2 transition">Forgot password?</button>
        </div>
        <div class="flex flex-col items-start justify-center mt-5 w-full">
            {#if error}
                <div class="text-sm mb-1 text-red-700">Error: {error}</div>
            {/if}
            <button onclick={() => goto("/auth/success")} class="py-2 flex items-center justify-center border border-ui-br w-full rounded-md text-ui-tx text-sm hover:bg-ui-bg-2 transition focus:outline-none focus:border focus:border-ui-tx-3">Sign In</button>
        </div>
