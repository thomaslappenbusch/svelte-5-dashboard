...
<div class="h-10 flex items-center justify-start mt-2 w-full">
	<div class="relative h-10 w-10 flex-shrink-0">
		...
		{#if data.statusColor}
			<div
				class="absolute right-0 bottom-[2px] h-[10px] w-[10px] {data.statusColor} rounded-full"
			></div>
		{/if}
	</div>
	{#if $isNavOpen}
		<div
			...
			class="flex flex-col justify-center items-start ml-3 min-w-0 flex-grow"
		>
